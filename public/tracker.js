...
(function() {
    ...
    function getElementSelector(el) {
        ...
        if (id) return `#${id}`;
        const classes = Array.from(el.classList).filter(c => c.length > 0).map(c => `.${c}`).join('');
    ...
    async function sendEvent(eventType, payload = {}) {
        ...
        try {
            ...
            if (navigator.sendBeacon && (eventType === 'page_unload' || eventType === 'session_end')) {
    ...
    function handleClick(event) {
        ...
        sendEvent('click', {
            x: event.clientX,
            y: event.clientY,
            x_relative: event.clientX / (document.documentElement.clientWidth || window.innerWidth),
            y_relative: event.clientY / (document.documentElement.clientHeight || window.innerHeight),
            element_id: target.id || '',
            element_classes: Array.from(target.classList || []).join(' ') || '',
            element_tag: target.tagName || '',
            element_text: target.textContent ? target.textContent.trim().substring(0, 255) : '', // Trim long text
